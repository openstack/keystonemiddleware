...
class AuditMiddlewareTest(base.BaseAuditMiddlewareTest):
    ...
    def test_api_request(self):
        self.create_simple_app().get(
            "/foo/bar", extra_environ=self.get_environ_header()
        )
    ...
    def test_cadf_event_scoped_to_request(self):
        ...
        self.assertEqual(
            self.notifier.calls[0].payload["id"], self.notifier.calls[1].payload["id"]
        )
    ...
    def test_missing_req(self):
        req = webob.Request.blank(
            "http://admin_host:8774/v2/" + str(uuid.uuid4()) + "/servers",
