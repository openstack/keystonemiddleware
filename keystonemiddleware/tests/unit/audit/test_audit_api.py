...
class AuditApiLogicTest(base.BaseAuditMiddlewareTest):
    def get_payload(self, method, url, audit_map=None, body=None, environ=None):
        audit_map = audit_map or self.audit_map
        environ = environ or self.get_environ_header()

        req = webob.Request.blank(
            url, body=body, method=method, environ=environ, remote_addr="192.168.0.1"
        )
    ...
    def test_get_list(self):
        ...
        self.assertEqual(
            payload["target"]["addresses"][0]["url"], "http://admin_host:8774"
        )
        ...
        self.assertEqual(
            payload["initiator"]["typeURI"], "service/security/account/user"
        )
        self.assertNotEqual(payload["initiator"]["credential"]["token"], "token")
        self.assertEqual(
            payload["initiator"]["credential"]["identity_status"], "Confirmed"
        )
    ...
    def test_get_read(self):
        url = "http://admin_host:8774/v2/%s/servers/%s" % (
            uuid.uuid4().hex,
            uuid.uuid4().hex,
        )
        payload = self.get_payload("GET", url)
    ...
    def test_post_action(self):
        ...
        body = (
            b'{"createImage" : {"name" : "new-image","metadata": '
    ...
    def test_custom_action(self):
        ...
        self.assertEqual(
            payload["target"]["typeURI"], "service/compute/os-hosts/host/reboot"
        )
    ...
    def test_response_mod_msg(self):
        url = "http://admin_host:8774/v2/" + str(uuid.uuid4()) + "/servers"
        req = webob.Request.blank(
            url, environ=self.get_environ_header("GET"), remote_addr="192.168.0.1"
        )
    ...
    def test_no_auth_token(self):
        ...
        # an exception.
        env_headers = {"HTTP_X_IDENTITY_STATUS": "Invalid", "REQUEST_METHOD": "GET"}
        ...
        self.assertEqual(payload["initiator"]["project_id"], taxonomy.UNKNOWN)
        self.assertEqual(payload["initiator"]["host"]["address"], "192.168.0.1")
        self.assertEqual(
            payload["initiator"]["typeURI"], "service/security/account/user"
        )
        self.assertNotEqual(payload["initiator"]["credential"]["token"], None)
        self.assertEqual(
            payload["initiator"]["credential"]["identity_status"], "Invalid"
        )
        self.assertNotIn("reason", payload)
        self.assertNotIn("reporterchain", payload)
        self.assertEqual(payload["observer"]["id"], "target")
