...
FAKE_ADMIN_TOKEN = jsonutils.dumps(
    {
        "access": {
            "token": {
                "id": FAKE_ADMIN_TOKEN_ID,
                "expires": "%i-10-03T16:58:01Z" % (1 + time.gmtime().tm_year),
            }
        }
    }
)
...
class TimeFixture(fixtures.Fixture):
    ...
    def setUp(self):
        ...
        ts_patcher = mock.patch.object(
            timeutils,
            "utcnow_ts",
            lambda microsecond=False: calendar.timegm(self.new_time.timetuple()),
        )
        ts_patcher.start()
...
class FakeApp(object):
    ...
    def __call__(self, req):
        ...
        if (
            req.environ.get("HTTP_X_IDENTITY_STATUS") == "Invalid"
            and req.environ["HTTP_X_SERVICE_IDENTITY_STATUS"] == "Invalid"
        ):
            ...
            self.need_service_token is True
            and req.environ.get("HTTP_X_SERVICE_TOKEN") is None
        ):
...
class v3FakeApp(FakeApp):
    ...
    def __init__(self, expected_env=None, need_service_token=False):
        ...
        super(v3FakeApp, self).__init__(
            expected_env=v3_default_env_additions, need_service_token=need_service_token
        )
...
class v3CompositeFakeApp(CompositeBase, v3FakeApp):
    ...
    def __init__(self, expected_env=None):
        ...
        super(v3CompositeFakeApp, self).__init__(v3_default_service_env_additions)
...
class GeneralAuthTokenMiddlewareTest(
    BaseAuthTokenMiddlewareTest, testresources.ResourcedTestCase
):
    ...
    def test_fixed_cache_key_length(self):
        ...
        self.assertThat(
            hashed_short_string_key, matchers.HasLength(len(hashed_long_string_key))
        )
    ...
    def test_conf_values_type_convert_with_wrong_value(self):
        ...
        self.assertRaises(
            ksm_exceptions.ConfigurationError,
            auth_token.AuthProtocol,
            self.fake_app,
            conf,
        )
...
class CommonAuthTokenMiddlewareTest(object):
    ...
    def test_request_invalid_uuid_token(self):
        ...
        resp = self.call_middleware(
            headers={"X-Auth-Token": "invalid-token"}, expected_status=401
        )
        self.assertEqual(
    ...
    def test_discovery_failure(self):
        ...
        with mock.patch.object(
            self.middleware._identity_server._adapter, "get_endpoint", return_value=None
        ):
    ...
    def test_http_request_max_retries(self):
        ...
        with mock.patch("time.sleep") as mock_obj:
            self.call_middleware(
                headers={"X-Auth-Token": ERROR_TOKEN},
                expected_status=503,
    ...
    def test_request_timeout(self):
        self.call_middleware(
            headers={"X-Auth-Token": TIMEOUT_TOKEN}, expected_status=503
        )
    ...
    def test_nocatalog(self):
        ...
        self.assert_valid_request_200(
            self.token_dict["uuid_token_default"], with_catalog=False
        )
    ...
    def test_uuid_bind_token_disabled_with_kerb_user(self):
        for use_kerberos in [True, False]:
            self.assert_kerberos_bind(
                self.token_dict["uuid_token_bind"],
                bind_level="disabled",
    ...
    def test_uuid_bind_token_permissive_with_unknown_bind(self):
        ...
        for use_kerberos in [True, False]:
            self.assert_kerberos_bind(
                token, bind_level="permissive", use_kerberos=use_kerberos, success=True
            )
    ...
    def test_uuid_bind_token_strict_with_unknown_bind(self):
        ...
        for use_kerberos in [True, False]:
            self.assert_kerberos_bind(
                token, bind_level="strict", use_kerberos=use_kerberos, success=False
            )
    ...
    def test_uuid_bind_token_required_with_unknown_bind(self):
        ...
        for use_kerberos in [True, False]:
            self.assert_kerberos_bind(
                token, bind_level="required", use_kerberos=use_kerberos, success=False
            )
    ...
    def test_uuid_bind_token_named_kerberos_with_unknown_bind(self):
        ...
        for use_kerberos in [True, False]:
            self.assert_kerberos_bind(
                token, bind_level="kerberos", use_kerberos=use_kerberos, success=False
            )
    ...
    def test_uuid_bind_token_with_unknown_named_FOO(self):
        ...
        for use_kerberos in [True, False]:
            self.assert_kerberos_bind(
                token, bind_level="FOO", use_kerberos=use_kerberos, success=False
            )
    ...
    def test_auth_plugin(self):
        for service_url in (
            self.examples.UNVERSIONED_SERVICE_URL,
    ...
    def test_service_token_with_valid_service_role_not_required(self):
        ...
        resp = self.call_middleware(
            headers={"X-Auth-Token": user_token, "X-Service-Token": service_token}
        )
...
def request_timeout_response(request, context):
    raise ksa_exceptions.RequestTimeout("Request to https://host/token/path timed out")
...
class v3AuthTokenMiddlewareTest(
    ...
    def setUp(self):
        super(v3AuthTokenMiddlewareTest, self).setUp(
            auth_version="v3.0", fake_app=v3FakeApp
        )
        ...
        self.token_dict = {
            ...
            "uuid_token_unknown_bind": self.examples.v3_UUID_TOKEN_UNKNOWN_BIND,
            "uuid_service_token_default": self.examples.v3_UUID_SERVICE_TOKEN_DEFAULT,
        }
    ...
    def test_valid_unscoped_uuid_request(self):
        ...
        self.assert_valid_request_200(
            self.examples.v3_UUID_TOKEN_UNSCOPED, with_catalog=False
        )
        self.assertLastPath("/v3/auth/tokens")
    ...
    def test_user_plugin_token_properties(self):
        ...
        self.assertEqual(
            token_data.project_domain_id, token_auth.user.project_domain_id
        )
        ...
        self.assertEqual(
            service_data.project_domain_id, token_auth.service.project_domain_id
        )
    ...
    def test_endpoint_not_found_in_token(self):
        ...
        self.middleware._token_cache.initialize({})
        with mock.patch.object(
            self.middleware._identity_server, "invalidate", new=mock.Mock()
        ):
            self.assertRaises(
                ksa_exceptions.EndpointNotFound, self.middleware.fetch_token, token
            )
    ...
    def test_app_cred_access_rules_service_request(self):
        ...
        headers = {"X-Auth-Token": token}
        self.call_middleware(
            headers=headers, expected_status=401, method="GET", path="/v2/images"
        )
        service_token = self.examples.v3_UUID_SERVICE_TOKEN_DEFAULT
        headers["X-Service-Token"] = service_token
        self.call_middleware(
            headers=headers, expected_status=200, method="GET", path="/v2/images"
        )
    ...
    def test_app_cred_no_access_rules_token(self):
        ...
        self.call_middleware(
            headers=headers, expected_status=401, method="GET", path="/v2.1/servers"
        )
    ...
    def test_app_cred_matching_rules(self):
        ...
        self.call_middleware(
            ...
            expected_status=200,
            method="GET",
            path="/v2.1/servers/foobar",
        )
        self.call_middleware(
            headers={"X-Auth-Token": token},
            expected_status=401,
            method="GET",
            path="/v2.1/servers/foobar/barfoo",
        )
        self.set_middleware(conf={"service_type": "image"})
        self.call_middleware(
            headers={"X-Auth-Token": token},
            expected_status=200,
            method="GET",
            path="/v2/images/foobar",
        )
        self.call_middleware(
            headers={"X-Auth-Token": token},
            expected_status=401,
            method="GET",
            path="/v2/images/foobar/barfoo",
        )
...
class DelayedAuthTests(BaseAuthTokenMiddlewareTest):
    ...
    def test_header_in_401(self):
        ...
        middleware = self.create_simple_middleware(
            status="401 Unauthorized", body=body, conf=conf
        )
        resp = self.call(middleware, expected_status=401)
        self.assertEqual(body.encode(), resp.body)

        self.assertEqual(
            'Keystone uri="%s"' % www_authenticate_uri, resp.headers["WWW-Authenticate"]
        )
...
class CommonCompositeAuthTests(object):
    ...
    def test_composite_auth_ok(self):
        ...
        roles = ",".join(
            [expected_env["HTTP_X_SERVICE_ROLES"], expected_env["HTTP_X_ROLES"]]
        )
    ...
    def test_composite_auth_no_user_token(self):
        service_token = self.token_dict["uuid_service_token_default"]
        resp = self.call_middleware(
            headers={"X-Service-Token": service_token}, expected_status=401
        )
        expected_body = b"The request you have made requires authentication."
        self.assertThat(resp.body, matchers.Contains(expected_body))
    ...
    def test_composite_auth_delay_no_user_token(self):
        ...
        service_token = self.token_dict["uuid_service_token_default"]
        resp = self.call_middleware(
            headers={"X-Service-Token": service_token}, expected_status=403
        )
        self.assertEqual(FakeApp.FORBIDDEN, resp.body)
...
class v3CompositeAuthTests(
    ...
    def setUp(self):
        super(v3CompositeAuthTests, self).setUp(
            auth_version="v3", fake_app=v3CompositeFakeApp
        )
...
class OtherTests(BaseAuthTokenMiddlewareTest):
    ...
    def test_unknown_server_versions(self):
        ...
        with mock.patch.object(
            ...
            self.call_middleware(
                headers={"X-Auth-Token": uuid.uuid4().hex}, expected_status=503
            )
...
class AuthProtocolLoadingTests(BaseAuthTokenMiddlewareTest):
    ...
    def good_request(self, app):
        ...
        self.requests_mock.post(
            "%s/v3/auth/tokens" % self.AUTH_URL,
            json=admin_token,
        ...
        request_headers = {
            "X-Subject-Token": user_token_id,
            "X-Auth-Token": admin_token_id,
        }

        self.requests_mock.get(
            "%s/v3/auth/tokens" % self.KEYSTONE_BASE_URL,
    ...
    def test_invalid_plugin_fails_to_initialize(self):
        ...
        self.assertRaises(
            ksa_exceptions.NoMatchingPlugin, self.create_simple_middleware
        )
...
class TestAuthPluginUserAgentGeneration(BaseAuthTokenMiddlewareTest):
    def setUp(self):
        ...
        # configure the authentication options
        self.conf = {"www_authenticate_uri": BASE_URI}
    ...
    def _assert_user_agent(self, app, project, ksm_version):
        ...
        expected_ua = "{0}keystonemiddleware.auth_token/{1}".format(
            project, ksm_version
        )
