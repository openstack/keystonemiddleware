...
class BaseAuthProtocolTests(testtools.TestCase):
    ...
    def test_process_flow(self, process_request, process_response):
        ...
        self.assertIsInstance(
            process_request.call_args[0][0], _request._AuthTokenRequest
        )
    ...
    def test_expired_user_token(self):
        t = fixture.V3Token()
        t.set_project_scope()
        t.expires = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(
            minutes=10
        )
    ...
    def test_good_v3_service_token(self):
        ...
        m = FetchingMiddleware(_do_cb, token_dict, service_token_roles=["service"])
    ...
    def test_base_doesnt_block_project_id(self):
        ...
        m = FetchingMiddleware(_do_cb)
        resp = self.call(
            m, headers={"X-Project-Id": project_id, "X-Domain-Id": domain_id}
        )
