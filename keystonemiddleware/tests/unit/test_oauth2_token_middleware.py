...
class Oauth2TokenMiddlewareTest(
    BaseOauth2TokenMiddlewareTest, testresources.ResourcedTestCase
):
    resources = [("examples", client_fixtures.EXAMPLES_RESOURCE)]

    def setUp(self):
        super(Oauth2TokenMiddlewareTest, self).setUp(
            auth_version="v3.0", fake_app=FakeOauth2TokenV3App
        )
    ...
    def test_request_blank_token(self):
        resp = self.call_middleware(
            headers=get_authorization_header(""), expected_status=401
        )
        self.assertEqual(
            'Keystone uri="https://keystone.example.com:1234"',
            resp.headers["WWW-Authenticate"],
    ...
    def test_request_not_app_cred_token(self):
        self.call_middleware(
            headers=get_authorization_header(self.examples.v3_UUID_TOKEN_DEFAULT),
            expected_status=200,
        )
    ...
    def test_http_error_not_cached_token(self):
        ...
        self.call_middleware(
            headers=get_authorization_header(ERROR_TOKEN), expected_status=503
        )
