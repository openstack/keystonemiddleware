...
from keystonemiddleware.tests.unit.auth_token.test_auth_token_middleware import FakeApp
...
class FakeOauth2TokenV3App(v3FakeApp):
    def __init__(
        self, expected_env=None, need_service_token=False, app_response_status_code=200
    ):
        super(FakeOauth2TokenV3App, self).__init__(expected_env, need_service_token)
...
class BaseExternalOauth2TokenMiddlewareTest(
    base.BaseAuthTokenTestCase, testresources.ResourcedTestCase
):
    ...
    def call_middleware(self, pem_client_cert=None, der_client_cert=None, **kwargs):
        if pem_client_cert:
            # apache
    ...
    def _introspect_response(
        ...
        if auth_method == "tls_client_auth":
            ...
            body = (
                "client_id=%s&client_secret=%s&token=%s&token_type_hint=access_token"
            ) % (introspect_client_id, introspect_client_secret, access_token)
            self.assertEqual(request.text, body)
            ...
            auth = "Basic " + base64.standard_b64encode(
                ("%s:%s" % (introspect_client_id, introspect_client_secret)).encode(
                    "ascii"
                )
            ).decode("ascii")
            ...
            self.assertIn(
                (
                    "client_assertion_type=urn%3Aietf%3Aparams%3A"
    ...
    def _check_env_value_project_scope(
        self,
        request_environ,
        user_id,
        user_name,
        ...
        self.assertEqual(project_domain_id, request_environ["HTTP_X_PROJECT_DOMAIN_ID"])
        self.assertEqual(
            project_domain_name, request_environ["HTTP_X_PROJECT_DOMAIN_NAME"]
        )
    ...
    def _check_env_value_domain_scope(
        self,
        request_environ,
        user_id,
        user_name,
    ...
    def _check_env_value_system_scope(
        self,
        request_environ,
        user_id,
        user_name,
...
class ExternalOauth2TokenMiddlewareTlsClientAuthTest(
    ...
    def setUp(self):
        ...
        self._default_metadata = {
            ...
            "user_domain": {"id": self._user_domain_id, "name": self._user_domain_name},
            "roles": self._roles,
...
class ExternalOauth2TokenMiddlewareClientSecretJWTTest(
    ...
    def test_fetch_token_introspect_response_active_is_false_401(self):
        ...
        self.requests_mock.post(
            self._introspect_endpoint, json=mock_resp, status_code=500
        )
        self.requests_mock.get(self._auth_url, json=VERSION_LIST_v3, status_code=300)
...
class ExternalOauth2TokenMiddlewareClientSecretBasicTest(
    ...
    def test_process_response_401(self):
        ...
        self.set_middleware(conf=conf)
        self.middleware = external_oauth2_token.ExternalAuth2Protocol(
            FakeOauth2TokenV3App(
                expected_env=self.expected_env, app_response_status_code=401
            ),
            self.conf,
        )
    ...
    def test_spoofed_headers_are_sanitized(self):
        ...
        self.assertEqual(self._user_domain_name, env["HTTP_X_USER_DOMAIN_NAME"])
...
class FilterFactoryTest(utils.BaseTestCase):
    def test_filter_factory(self):
        ...
        self.assertEqual(certfile, m._get_config_option("certfile", is_required=False))
        self.assertEqual(keyfile, m._get_config_option("keyfile", is_required=False))
        self.assertEqual(cafile, m._get_config_option("cafile", is_required=False))
        self.assertEqual(insecure, m._get_config_option("insecure", is_required=False))
        self.assertEqual(
            http_connect_timeout,
            m._get_config_option("http_connect_timeout", is_required=False),
        )
        self.assertEqual(
            introspect_endpoint,
            m._get_config_option("introspect_endpoint", is_required=False),
        )
        self.assertEqual(audience, m._get_config_option("audience", is_required=False))
        self.assertEqual(
            auth_method, m._get_config_option("auth_method", is_required=False)
        )
        self.assertEqual(
            client_id, m._get_config_option("client_id", is_required=False)
        )
        self.assertEqual(
            client_secret, m._get_config_option("client_secret", is_required=False)
        )
        self.assertEqual(
            thumbprint_verify,
            m._get_config_option("thumbprint_verify", is_required=False),
        )
        self.assertEqual(
            jwt_key_file, m._get_config_option("jwt_key_file", is_required=False)
        )
        self.assertEqual(
            jwt_algorithm, m._get_config_option("jwt_algorithm", is_required=False)
        )
        self.assertEqual(
            jwt_bearer_time_out,
            m._get_config_option("jwt_bearer_time_out", is_required=False),
        )
        self.assertEqual(
            mapping_project_id,
            m._get_config_option("mapping_project_id", is_required=False),
        )
        self.assertEqual(
            mapping_project_name,
            m._get_config_option("mapping_project_name", is_required=False),
        )
        self.assertEqual(
            mapping_project_domain_id,
            m._get_config_option("mapping_project_domain_id", is_required=False),
        )
        self.assertEqual(
            mapping_project_domain_name,
            m._get_config_option("mapping_project_domain_name", is_required=False),
        )
        self.assertEqual(
            mapping_user_id, m._get_config_option("mapping_user_id", is_required=False)
        )
        self.assertEqual(
            mapping_user_name,
            m._get_config_option("mapping_user_name", is_required=False),
        )
        self.assertEqual(
            mapping_user_domain_id,
            m._get_config_option("mapping_user_domain_id", is_required=False),
        )
        self.assertEqual(
            mapping_user_domain_name,
            m._get_config_option("mapping_user_domain_name", is_required=False),
        )
        self.assertEqual(
            mapping_roles, m._get_config_option("mapping_roles", is_required=False)
        )
