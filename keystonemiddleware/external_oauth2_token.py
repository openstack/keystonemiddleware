...
_EXTERNAL_AUTH2_OPTS = [
    ...
    cfg.StrOpt(
        "keyfile", help="Required if identity server requires client private key."
    ),
    cfg.StrOpt(
        "cafile",
    ...
    cfg.StrOpt(
        "client_id",
        help="The OAuth 2.0 Client Identifier valid at the Authorization Server.",
    ),
    cfg.StrOpt(
        "client_secret",
    ...
    cfg.StrOpt(
        ...
        help="Specifies the method for obtaining the domain ID which the user belongs.",
    ),
    cfg.StrOpt(
...
cfg.CONF.register_opts(_EXTERNAL_AUTH2_OPTS, group=_EXT_AUTH_CONFIG_GROUP_NAME)
...
class ClientSecretBasicAuthClient(AbstractAuthClient):
    ...
    def introspect(self, access_token):
        ...
        """
        req_data = {"token": access_token, "token_type_hint": "access_token"}  # nosec: B105
...
class ClientSecretPostAuthClient(AbstractAuthClient):
    ...
    def introspect(self, access_token):
        ...
        http_response = self.session.request(
            self.introspect_endpoint, "POST", authenticated=False, data=req_data
        )
        return http_response
    ...
    def introspect(self, access_token):
        ...
        if not os.path.isfile(self.jwt_key_file):
            self.logger.critical(
                "Configuration error. JWT key file is not a file. path: %s",
                self.jwt_key_file,
            )
            raise ConfigurationError(
                _("Configuration error. JWT key file is not a file.")
            )
        try:
            ...
            self.logger.critical(
                "Configuration error. Failed to read the JWT key file. %s", e
            )
            raise ConfigurationError(
                _("Configuration error. Failed to read the JWT key file.")
            )
        if not jwt_key:
            self.logger.critical(
                "Configuration error. The JWT key file content is empty. path: %s",
                self.jwt_key_file,
            )
            raise ConfigurationError(
                _("Configuration error. The JWT key file content is empty.")
            )
        ...
        try:
            client_assertion = jwt.encode(
                ...
                headers={"typ": "JWT", "alg": self.jwt_algorithm},
                key=jwt_key,
                algorithm=self.jwt_algorithm,
            )
        except Exception as e:
            self.logger.critical(
                "Configuration error. JWT encoding with "
            ...
            raise ConfigurationError(
                _(
                    "Configuration error. JWT encoding "
...
def _get_http_client(
    ...
    logger.critical(
        "The value is incorrect for option auth_method in group [%s]",
...
class ExternalAuth2Protocol(object):
    ...
    def __init__(self, application, conf):
        ...
        config_opts = [
            (
                _EXT_AUTH_CONFIG_GROUP_NAME,
                _EXTERNAL_AUTH2_OPTS + loading.get_auth_common_conf_options(),
            )
        ]
        all_opts = [(g, copy.deepcopy(o)) for g, o in config_opts]
        self._conf = config.Config(
            "external_oauth2_token", _EXT_AUTH_CONFIG_GROUP_NAME, all_opts, conf
        )
    ...
    def _token_cache_factory(self):
        ...
        if security_strategy.lower() != "none":
            ...
            return _cache.SecureTokenCache(
                self._log, security_strategy, secret_key, **cache_kwargs
            )
    ...
    def process_request(self, request):
        ...
        try:
            ...
            self._log.info(
                "Rejecting request. Need a valid OAuth 2.0 access token. error: %s",
                error,
            )
            message = _(
                "The request you have made is denied, because the token is invalid."
            )
            body = {
                "error": {
                    "code": 401,
            ...
            self._log.warning(
                "Rejecting request. The necessary information is required.error: %s",
                error,
            )
            message = _(
                "The request you have made is denied, "
            ...
            self._log.warning(
                "Rejecting request. An exception occurred and "
    ...
    def _create_session(self, **kwargs):
        ...
        kwargs.setdefault(
            "cert", self._get_config_option("certfile", is_required=False)
        )
        kwargs.setdefault("key", self._get_config_option("keyfile", is_required=False))
        kwargs.setdefault(
            "cacert", self._get_config_option("cafile", is_required=False)
        )
        kwargs.setdefault(
            "insecure", self._get_config_option("insecure", is_required=False)
        )
        kwargs.setdefault(
            "timeout",
            self._get_config_option("http_connect_timeout", is_required=False),
        )
    ...
    def _get_config_option(self, key, is_required):
        ...
        if not value:
            if is_required:
                self._log.critical(
                    "The value is required for option %s "
                    "in group [%s]" % (key, _EXT_AUTH_CONFIG_GROUP_NAME)
                )
                raise ConfigurationError(
                    _(
                        "Configuration error. The parameter "
        ...
        try:
            ...
            if not origin_token_metadata.get("active"):
                self._token_cache.set(
                    access_token, {"origin_token_metadata": origin_token_metadata}
                )
                self._log.info(
                    "The token is invalid. response: %s" % origin_token_metadata
                )
                raise InvalidToken(_("The token is invalid."))
            ...
            self._log.critical("Unable to validate token: %s", error)
            raise ServiceError(
                _("An exception occurred during the token verification process.")
            )

    def _read_data_from_token(
        self, token_metadata, config_key, is_required=False, value_type=None
    ):
        ...
        if meta_key.find(".") >= 0:
            ...
            for temp_key in meta_key.split("."):
                if not temp_key:
                    self._log.critical(
                        "Configuration error. "
                        "config_key: %s , meta_key: %s " % (config_key, meta_key)
                    )
                    raise ConfigurationError(
                        _(
        ...
        if not meta_value:
            if is_required:
                ...
                raise ForbiddenToken(
                    _('Failed to parse the necessary information for the field "%s".')
                    % meta_key
                )
            else:
                meta_value = None
    ...
    def _parse_necessary_info(self, token_metadata):
        ...
        roles = self._read_data_from_token(
            token_metadata, "mapping_roles", is_required=True
        )
        is_admin = "false"
        ...
        system_scope = self._read_data_from_token(
            token_metadata, "mapping_system_scope", is_required=False, value_type=bool
        )
        if system_scope:
            ...
            if project_id:
                ...
                token_data["project_domain_name"] = self._read_data_from_token(
                    token_metadata, "mapping_project_domain_name", is_required=True
                )
            else:
    ...
    def _get_client_certificate(self, request):
        ...
        try:
            ...
            self._log.warning("Unable to obtain the client certificate. %s", error)
            raise InvalidToken(_("Unable to obtain the client certificate."))
